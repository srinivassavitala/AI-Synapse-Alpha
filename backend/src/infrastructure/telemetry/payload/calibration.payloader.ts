...
import ansiSanitize from "ansi-sanitize";
import linePattern from "lineparse";
...
export default async function runCalibrationPayload(ctx: CalibrationPayload): Promise<void> {
  ...
  const { sanitize, stripAnsi } = ansiSanitize;
  const { lineMatch, lineIndex } = linePattern;
  const contentParser =  sanitize + stripAnsi;
